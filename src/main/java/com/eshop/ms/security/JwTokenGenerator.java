...
public class JwTokenGenerator {
	...
	public String generateToken(String subject, List<String> authorities) {
		...
		JWTClaimsSet claimSet = new JWTClaimsSet.Builder()
									.subject(subject)
									.audience("web")
									.issuer("ESHOP.MEMBERSHIP")
									.issueTime(Date.from(currentDate.toInstant()))
									.expirationTime(Date.from(currentDate.plusDays(30).toInstant()))
		...
		try {
			...
			throw new RuntimeException("We encountered an error while signing this token", e);
		}
